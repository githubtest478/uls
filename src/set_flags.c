...
void set_flags(t_names *names, char c)
{
    ...
    else if(c == '1') { //the same grup as -1 -m
        RESET_FLAG(names->flags, flag_m);
    ...
    else if(c == 'C') { //the same grup as -x -C
        RESET_FLAG(names->flags, flag_x);
    ...
    else if(c == 'x') { //the same grup as -x -C
        RESET_FLAG(names->flags, flag_C);
        SET_FLAG(names->flags, flag_x);
    }
    else if(c == 'p') {
        SET_FLAG(names->flags, flag_p);
    }
    else if(c == 'm') { //the same grup as -1 -m
        RESET_FLAG(names->flags, flag_one);
